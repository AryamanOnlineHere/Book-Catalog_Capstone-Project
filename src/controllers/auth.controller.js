...
exports.signup = async (request, response) => {
  try {
    ...
    if (role === 'admin') {
      ...
      await admin.save();
      return response.status(201).json({ message: 'Admin registered successfully' });
    } else if (role === 'author') {
      const author = new Author({ username, password, email, role });
      await author.save();
      return response.status(201).json({ message: 'Author registered successfully' });
    } else {
      return response.status(400).json({ error: 'Invalid role. Use "admin" or "author".' });
    }
  } catch (err) {
